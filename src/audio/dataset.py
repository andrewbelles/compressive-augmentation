...
def _srht_cs_view(y: np.ndarray, ratio: float, rng: np.random.Generator) -> torch.Tensor:
...
def _audio_load_sample(ds: "WaveBarlowDataset | WaveABTDataset | SupConDataset", index: int) -> np.ndarray:
    """Shared implementation of load_sample for audio dataset classes."""
    epoch_seed = int(torch.initial_seed()) % (2 ** 31) if ds.is_train else 0
    rng    = np.random.default_rng([ds.seed, index, epoch_seed])
    offset = float(rng.uniform(10.0, 25.0))
    n      = int(ds.segment_seconds * ds.sample_rate)
    if ds._wav_cache is not None:
        y_full = ds._wav_cache[index]
        start  = int(offset * ds.sample_rate)
        seg    = y_full[start : start + n].astype(np.float32)
        if len(seg) < n:
            seg = np.pad(seg, (0, n - len(seg)))
        return seg
    return load_waveform(
        ds.audio_root / Path(ds.rows[index]["audio_path"]),
        ds.sample_rate, offset, ds.segment_seconds,
    )
...
class WaveBarlowDataset(BaseBarlowDataset):
    ...
    def load_sample(self, index: int) -> np.ndarray:
        ...
        return _audio_load_sample(self, index)
...
class SupConDataset(BaseBarlowDataset):
    ...
    def make_views(
        ...
        # SupConDataset returns a triple; make_views satisfies the base contract but is called via __getitem__
        y  = self.load_sample(index)
